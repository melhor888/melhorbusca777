    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-[50vh] md:h-[65vh] overflow-hidden">
        ...
        <div className="absolute top-4 left-4 z-20">
          <Link
            to={isProperty ? "/imoveis" : "/veiculos"}
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-4">
                <img
                  src={company.logo}
                  alt={company.name}
                  className="w-16 h-16 md:w-20 md:h-20 rounded-2xl object-cover border-2 border-card shadow-xl"
                />
                <div>
                  <h1 className="font-display font-bold text-2xl md:text-4xl text-foreground leading-tight">
                    {company.name}
                  </h1>
              ...
              <div className="flex gap-3 mt-5">
                <a
                  href={whatsappUrl(heroProduct?.title || company.name)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-[#25d366] to-[#128c7e] text-white font-bold text-sm hover:opacity-90 transition-opacity shadow-lg"
                >
                  <MessageCircle size={18} />
                  WhatsApp
                </a>
                <button
                  onClick={() => navigator.share?.({ title: company.name, url: window.location.href })}
                  className="flex items-center gap-2 px-5 py-3 rounded-xl bg-card/60 backdrop-blur-md text-foreground font-medium text-sm hover:bg-card/80 transition-colors"
                >
                  <Share2 size={16} />
                  Compartilhar
                </button>
      ...
      <section className="container max-w-6xl mx-auto px-4 py-8">
        ...
        {products.length > 0 ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-muted-foreground text-sm mt-1">Em breve novos itens serão adicionados</p>
          </div>
        )}
      </section>
    </div>
