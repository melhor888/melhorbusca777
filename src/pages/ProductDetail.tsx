...
export default function ProductDetail() {
  ...
  // For properties: use individual location. For vehicles: use company address.
  const mapAddress = isProperty && product.location ? product.location : company.address;
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative">
        <div className="aspect-[16/9] md:aspect-[21/9] overflow-hidden bg-muted">
          <motion.img key={activeImage} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }} src={product.images[activeImage]} alt={product.title} className="w-full h-full object-cover" />
        </div>
        ...
        {product.tag && (
          <div className="absolute top-4 right-4 z-20">
            <span className={`px-3 py-1.5 rounded-lg text-xs font-bold shadow-lg ${getTagStyle(product.tag)}`}>
              <Tag size={12} className="inline mr-1 mb-0.5" />{product.tag}
            </span>
          </div>
        )}
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <h1 className="font-display font-bold text-2xl md:text-4xl text-foreground mt-1 leading-tight">{product.title}</h1>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              ...
              <div className="bg-card border border-border rounded-2xl overflow-hidden">
                {Object.entries(product.specs).map(([key, value], i) => (
                  <div key={key} className={`flex items-center justify-between px-5 py-3.5 ${i % 2 === 0 ? "bg-card" : "bg-muted/50"} ${i < Object.entries(product.specs).length - 1 ? "border-b border-border" : ""}`}>
