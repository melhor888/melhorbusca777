...
const popularIngredients = [
  "tortilla de milho", "carne de porco", "carne moída", "frango desfiado",
  "cebola", "alho", "tomate", "pimenta jalapeño", "pimenta chipotle",
  "coentro", "limão", "abacate", "feijão preto", "feijão refrito",
  "queijo oaxaca", "queijo cotija", "creme de leite", "chili em pó",
  "cominho", "orégano mexicano", "achiote", "milho", "arroz"
];
  ...
  return (
    <div className="min-h-screen pb-24">
      ...
      <div className="sticky top-0 z-40 glass-card border-b border-border/50 px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="font-display font-bold text-foreground text-lg">O que tenho em casa? 🌶️</h1>
      </div>
      ...
      <div className="px-4 pt-6 max-w-lg mx-auto space-y-6">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Buscar ingrediente mexicano..."
        ...
        {selected.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {selected.map((s) => (
              <button key={s} onClick={() => toggle(s)} className="flex items-center gap-1 px-3 py-1.5 rounded-full bg-primary/20 text-primary text-xs font-semibold">
                {s} <X size={12} />
              </button>
            ))}
            <button onClick={() => setSelected([])} className="px-3 py-1.5 rounded-full bg-secondary text-muted-foreground text-xs">
              Limpar
            </button>
          </div>
        )}
        ...
        <div>
          <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold mb-3">
            {search ? "Resultados" : "🌮 Ingredientes populares da culinária mexicana"}
          </p>
          <div className="flex flex-wrap gap-2">
          ...
          <div>
            ...
            <div className="space-y-2">
              {matchedDishes.map(({ dish, matchPct }) => (
                ...
                >
                  <img src={getDishImage(dish.image)} alt={dish.name} className="w-14 h-14 rounded-lg object-cover flex-shrink-0" />
                  ...
                  <div className="text-right">
                    <span className={`text-xs font-bold ${matchPct >= 0.8 ? "text-green-400" : matchPct >= 0.5 ? "text-yellow-400" : "text-muted-foreground"}`}>
